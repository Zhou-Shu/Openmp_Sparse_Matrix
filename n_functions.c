...
Clist ScalarMuti(Clist M, int n)
{
    OLink p;
...
#pragma omp for private(p, q,flg)
    for (size_t i = 0; i < M1->m; i++)
    {
        ...
        while (q)// let M1=M1+M2
        {
            ...
            {
                p->n_value+=q->n_value;
                if (!p->n_value)// If addition=0, remove the node
                {
                    OLink s;
                    M1->len--;
                    if (!M1->row_head[i]->n_value)//when pointer is row_head, connecting next node directly 
                    {
...
Clist MatrixMuti(Clist M1, Clist M2)
{
    omp_init_lock(&lock);
...
#pragma omp parallel 
{
#pragma omp for private(q, p, q1, q2, n_value) collapse(2) 
...
{
            //omp_set_lock(&lock);
            //Setting lock which as same as "pragma omp critical" 
